...
Unter einem unveränderbaren Server versteht man also eine Ressource, die niemals verändert, sondern nur durch eine aktualisierte Version ersetzt wird \cite{ImmutableServer}. Auch die in diesem Kapitel betrachtete Containertechnologie \textit{Docker} basiert auf dieser Idee.
...
Unumstritten sind Container eines der zukunftsträchtigsten Technologien der letzten Jahre. Vor allem im Bereich von Cloud"=Computing haben sie sehr großes Potential. Die Zukunft wird zeigen, ob und wie sich Container zwischen allen bestehenden \textit{Plattform-as-a-Service} Angeboten platzieren kann. 
